...
// In dev, connect to backend on same host port 8000
...
export async function fetchItems(roomCode: string): Promise<GroceryItem[]> {
  const res = await fetch(`${API}/rooms/${roomCode}/items`);
...
export async function toggleItem(roomCode: string, id: number): Promise<GroceryItem> {
  const res = await fetch(`${API}/rooms/${roomCode}/items/${id}/toggle`, { method: 'POST' });
  if (!res.ok) throw new Error('Failed to toggle item');
  return res.json();
}

export async function deleteItem(roomCode: string, id: number): Promise<void> {
  const res = await fetch(`${API}/rooms/${roomCode}/items/${id}`, { method: 'DELETE' });
